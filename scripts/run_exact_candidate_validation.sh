...
record_dmg() {
  ...
  print -r -- "$label.dmg_path=$path" >> "$MANIFEST"
  print -r -- "$label.dmg_sha256=$actual" >> "$MANIFEST"
  print -r -- "$label.dmg_size=$size" >> "$MANIFEST"
  hdiutil verify "$path" || die "$label DMG hdiutil verify failed"
}

mount_and_copy() {
  local label="$1" path="$2" mount_point="$3" copy_path="$4"
  hdiutil attach -readonly -nobrowse -mountpoint "$mount_point" "$path" || die "$label DMG read-only mount failed"
  local app_count
